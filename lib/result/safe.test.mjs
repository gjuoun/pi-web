...
const { safeJsonParse, safeSync, safeReadFileText, safeRequestJson, safeAsync, trySync } = await createJiti(
  import.meta.url,
).import("./safe.ts");
...
test("trySync reports errno codes for fs failures", () => {
  assert.deepEqual(trySync(() => "fine"), { ok: true, value: "fine" });
  const missing = trySync(() => {
    throw Object.assign(new Error("ENOENT: no such file"), { code: "ENOENT" });
  });
  assert.equal(missing.ok, false);
  if (!missing.ok) {
    assert.equal(missing.code, "ENOENT");
    assert.equal(missing.message, "ENOENT: no such file");
  }
});
