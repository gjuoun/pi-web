...
test("the input normaliser keeps the spaces a controlled field must round-trip", () => {
  assert.equal(normalizeFontFamilyInput("Comic Sans"), "Comic Sans");
  assert.equal(normalizeFontFamilyInput("Comic "), "Comic ");
  assert.equal(normalizeFontFamilyInput(" Comic"), " Comic");
  assert.equal(normalizeFontFamilyInput("Comic  Sans"), "Comic Sans");
  assert.equal(normalizeFontFamilyInput(""), "");
  assert.equal(normalizeFontFamilyInput("   "), " ");
  assert.equal(normalizeFontFamilyInput(undefined), "");
  assert.equal(normalizeFontFamilyInput(42), "");
  assert.equal(normalizeFontFamilyInput("Foo; color: red"), "Foo color red");
  assert.equal(normalizeFontFamilyInput("a".repeat(500)).length, 200);
});

test("sanitize still trims what the normaliser deliberately preserves", () => {
  assert.equal(sanitizeFontFamily(" Comic "), "Comic");
  assert.equal(sanitizeFontFamily(normalizeFontFamilyInput("Comic Sans ")), "Comic Sans");
  assert.equal(composeFontStack("ui", normalizeFontFamilyInput("Inter ")), `Inter, ${UI_FONT_FALLBACK_TAIL}`);
});
