...
test("keeps the pi-subagents package loaded next to the integrated extension", () => {
  const host = {
    path: "<inline:pi-web-subagents>",
    tools: new Map([["Agent", {}], ["get_subagent_result", {}], ["steer_subagent", {}]]),
  };
  const pkg = {
    path: "/Users/me/.pi/agent/npm/node_modules/@tintinweb/pi-subagents/src/index.ts",
    sourceInfo: { source: "npm:@tintinweb/pi-subagents" },
    tools: new Map([["Agent", {}], ["SubagentWorkflow", {}], ["get_subagent_result", {}], ["steer_subagent", {}]]),
  };
  const base = { extensions: [pkg, host], errors: [] };
  // Nothing is removed: both engines are expected to run at once (docs/adr/0003).
  assert.equal(suppressExpectedSubagentConflicts(base), base);
});

test("drops the expected tool conflicts with the package and keeps every other error", () => {
  const host = { path: "<inline:pi-web-subagents>", tools: new Map([["Agent", {}]]) };
  const pkgPath = "/Users/me/.pi/agent/npm/node_modules/@tintinweb/pi-subagents/src/index.ts";
  const pkg = {
    path: pkgPath,
    sourceInfo: { source: "npm:@tintinweb/pi-subagents" },
    tools: new Map([["Agent", {}], ["get_subagent_result", {}], ["steer_subagent", {}]]),
  };
  const unrelated = { path: "/tmp/other.ts", tools: new Map([["search", {}]]) };
  const errors = [
    { path: host.path, error: `Tool "Agent" conflicts with ${pkgPath}` },
    { path: host.path, error: `Tool "get_subagent_result" conflicts with ${pkgPath}` },
    { path: host.path, error: `Tool "steer_subagent" conflicts with ${pkgPath}` },
    // A duplicated copy of the package must stay visible: it means every RPC answers twice.
    { path: pkgPath, error: `Flag "--subagents-workflow-file" conflicts with ${pkgPath}` },
    { path: unrelated.path, error: "something else" },
  ];
  const result = suppressExpectedSubagentConflicts({ extensions: [pkg, host, unrelated], errors });
  assert.deepEqual(result.errors.map((error) => error.error), [
    `Flag "--subagents-workflow-file" conflicts with ${pkgPath}`,
    "something else",
  ]);
  assert.deepEqual(result.extensions, [pkg, host, unrelated]);
});

test("recognises the package from a scoped npm source or from its path", () => {
  assert.equal(isPiSubagentsPackageExtension({ path: "/x/index.ts", sourceInfo: { source: "npm:@tintinweb/pi-subagents" } }), true);
  assert.equal(isPiSubagentsPackageExtension({ path: "/tmp/pi-subagents/index.ts" }), true);
  assert.equal(isPiSubagentsPackageExtension({ path: "/tmp/other.ts", sourceInfo: { source: "npm:other" } }), false);
});
...
test("leaves a base without the package untouched", () => {
  const host = { path: "<inline:pi-web-subagents>", tools: new Map([["Agent", {}]]) };
  const partial = { path: "/tmp/partial.ts", tools: new Map([["Agent", {}]]) };
  const base = {
    extensions: [host, partial],
    errors: [{ path: host.path, error: `Tool "Agent" conflicts with ${partial.path}` }],
  };
  assert.equal(suppressExpectedSubagentConflicts(base), base);
});
