...
test("the package bridge is the only sub-agent engine a session loads", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  // One factory, and it only observes: pi-web runs no children of its own (docs/adr/0006).
  assert.match(startupSource, /extensionFactories: \[\s*(\/\/[^\n]*\n\s*)*createPiSubagentsBridgeExtension\(\{/);
  assert.doesNotMatch(source, /createSubagentExtension|createSubagentController|suppressExpectedSubagentConflicts/);
  assert.doesNotMatch(source, /isBuiltInSubagentsEnabled|listSubagentProfiles/);
  assert.doesNotMatch(startupSource, /extensionsOverride:/);
});

test("a reopened sub-agent session keeps its resource policy and cannot nest", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  ...
  // The package registers a fourth tool, so the no-nesting exclusion must cover it too.
  assert.match(startupSource, /excludeTools: \[\.\.\.SUBAGENT_CONTROL_TOOL_NAMES, "SubagentWorkflow"\]/);
  assert.match(startupSource, /const toolsOption: string\[\] \| undefined = subagentResources\?\.tools/);
...
test("RPC session startup treats only sessions with messages as continuing", async () => {
  ...
  assert.match(startupSource, /const initial = restoredModelUsable \? null :/);
...
test("RPC session startup falls back to another authenticated model when the restored one has none", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  // The fallback-to-any-visible-model resolver must be gated on whether the restored model is
  // actually usable (has configured auth), not merely on whether the session already has
  // messages — otherwise a reopened session whose recorded provider lost auth gets no model at
  // all passed to the SDK, and the SDK's own "unknown"/"unknown" sentinel surfaces in the UI.
  assert.match(
    startupSource,
    /const restoredModelUsable = Boolean\(restoredModel && services\.modelRuntime\.hasConfiguredAuth\(restoredModel\.provider\)\)/,
  );
  assert.match(startupSource, /const initial = restoredModelUsable \? null : selectInitialModelScope\(/);
  assert.match(startupSource, /const startupModel = restoredModelUsable\s*\n?\s*\? restoredModel\s*\n?\s*: initial\?\.model;/);
  assert.doesNotMatch(startupSource, /const initial = hasExistingMessages \? null :/);
});
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(deleteRouteSource, /getRpcSession\(id\)\?\.shutdown\(\)/);
...
test("pi-web registers no bash tool of its own, so pi's configuration decides alone", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  // The built-in bash tool is pi's; pi-web neither shadows it nor re-activates it by hand.
  assert.doesNotMatch(source, /createProjectCommandBashExtension|preferUserBashExtension/);
  assert.doesNotMatch(startupSource, /setActiveToolsByName/);
  assert.match(startupSource, /const toolsOption: string\[\] \| undefined = subagentResources\?\.tools/);
});
...
test("a normal session carries no tool policy: only a subagent profile pins tools", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  ...
  // The only tool allowlist left is the subagent profile's snapshot.
  assert.match(startupSource, /const toolsOption: string\[\] \| undefined = subagentResources\?\.tools/);
  assert.doesNotMatch(source, /readSessionToolSelection|appendSessionToolSelection|setActiveToolSelection|withExtensionTools/);
  // Chat only survives as a subagent-profile policy only.
  assert.match(
    startupSource,
    /const chatOnly = Boolean\(\s*subagentResources && subagentResources\.tools\.length === 0 && !subagentLoadsResources,/,
  );
  assert.ok(startupSource.indexOf("const chatOnly") < startupSource.indexOf("createAgentSessionServices("));
  assert.match(startupSource, /const trustReloadOptions = subagentResources && !subagentLoadsResources[\s\S]*?projectTrustReloadOptions\(sessionCwd, agentDir\)/);
