...
test("a normal session's active built-ins follow pi's configuration, not the host bash extension", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));

  // The project-command bash tool is registered as an extension tool, so the SDK activates it
  // even when `defaultTools` leaves bash out. Strip it so a pi-web session matches the `pi` CLI.
  assert.match(startupSource, /const configuredTools = settingsManager\.getDefaultTools\(\)/);
  assert.match(startupSource, /if \(configuredTools && !configuredTools\.includes\("bash"\) && active\.includes\("bash"\)\)/);
  assert.match(startupSource, /inner\.setActiveToolsByName\(active\.filter\(\(name\) => name !== "bash"\)\)/);
  // Subagents pin their own list through `toolsOption`, so they are left alone.
  assert.match(startupSource, /if \(!subagentResources\) \{\s*const configuredTools/);
});
