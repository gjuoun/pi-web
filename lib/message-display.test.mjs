...
test("takes the newest thinking line for the collapsed chip without splitting Unicode characters", async () => {
  const { getThinkingTail, THINKING_TAIL_MAX_CHARS } = await loadSubject();

  assert.equal(getThinkingTail("First line\r\n\r\n  Detailed second line.  "), "Detailed second line.");
  assert.equal(getThinkingTail("Only one line"), "Only one line");
  // Trailing blank lines must not hide the last line that was actually written.
  assert.equal(getThinkingTail("First line\n\n \n\t"), "First line");
  assert.equal(getThinkingTail(" \n\t"), "");
  assert.equal(getThinkingTail(""), "");
  // The chip scrolls to the end, so an over-long line is capped from the head, code point safe.
  assert.equal(getThinkingTail("a".repeat(500), 10), "a".repeat(10));
  assert.equal(getThinkingTail("\u{1F4A1}".repeat(300), 240), "\u{1F4A1}".repeat(240));
  assert.ok(THINKING_TAIL_MAX_CHARS >= 240);
});
