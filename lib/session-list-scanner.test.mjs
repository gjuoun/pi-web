...
const { SESSION_PINNED_TYPE, SESSION_ARCHIVED_TYPE } = await jiti.import("./session-flags.ts");
...
test("pinned/archived custom entries flow from disk to ScannedSessionInfo and SessionInfo", async (t) => {
  const { write } = fixture(t);
  const a = write("a", [message("a1", "user", "first")]);
  SessionManager.open(a).appendCustomEntry(SESSION_PINNED_TYPE, true);
  SessionManager.open(a).appendCustomEntry(SESSION_ARCHIVED_TYPE, true);
  const scanned = await listSessionsIncremental();
  const scannedA = scanned.find((s) => s.id === "a");
  assert.equal(scannedA.pinned, true);
  assert.equal(scannedA.archived, true);

  const all = await listAllSessions();
  const allA = all.find((s) => s.id === "a");
  assert.equal(allA.pinned, true);
  assert.equal(allA.archived, true);
});
