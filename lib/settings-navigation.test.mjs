...
test("restores the last settings section and falls back without a project", () => {
  ...
  setLastSettingsSection("plugins", storage);
  assert.equal(getLastSettingsSection("/project", storage), "plugins");

  // A section stored before the sub-agent panel was removed falls back instead of resolving.
  storage.setItem("pi-web:settings-navigation", JSON.stringify({ section: "agents" }));
