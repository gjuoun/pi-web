...
// Type-only: the bridge reaches the pi SDK, which must never enter the browser bundle.
...
import type { BlockingExtensionUiRequest, SessionInfo, SessionTreeNode, ToolEntry } from "@/lib/types";
...
/** The top-bar dropdowns. `agents` is the one sub-agent surface (docs/adr/0006). */
type TopPanelId = "agents" | "branches" | "system" | "tools" | "session";
...
export function AppShell() {
  ...
  /**
   * The single Agents tab's live feed: runs of the `pi-subagents` engine for the selected session
   * (docs/adr/0006). Polled while the tab is closed too, because the badge needs the count.
   */
  const [piSubagentRuns, setPiSubagentRuns] = useState<readonly PiSubagentRun[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const data = await response.json() as { runs?: PiSubagentRun[] };
        if (cancelled) return;
        setPiSubagentRuns(Array.isArray(data.runs) ? data.runs : []);
  ...
  const runningPiSubagentRuns = piSubagentRuns.filter((run) =>
    run.status === "running" || run.status === "starting" || run.status === "steered").length;
  // A run the bridge has not attached yet has no session on disk, so the tab must count it too.
  const pendingPiSubagentRuns = piSubagentRuns.filter((run) => !run.childSessionId).length;
  const agentTabCount = (activeSessionFamily?.subagents.length ?? 0) + pendingPiSubagentRuns;
  const hasAgentSurface = hasSubagentSessions || piSubagentRuns.length > 0;
